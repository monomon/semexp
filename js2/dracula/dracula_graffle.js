        ...
        draw : function() {
            ...
            style && style.label 
                && (edge.label && edge.label.attr({x:(x1+x4)/2, y:(y1+y4)/2}) 
                    || (edge.label = selfRef.set().push(selfRef.rect((x1+x4)/2, (y1+y4)/2, 40, 40).attr({fill:"#ccc", stroke:0})).push(selfRef.text((x1+x4)/2, (y1+y4)/2, style.label).attr({fill: "#000", "font-size": style["font-size"] || "12px"}))));
